...
export function buildStandardEngineCatalog({
  ...
  const externalEngines = safeArray(externals).map((item) => {
    ...
    return {
      ...
      executionMode: safeText(item.executionMode),
      packageName: safeText(item.packageName),
      cliModule: safeText(item.cliModule),
      entrypoint: safeText(item.entrypoint),
      localAlgorithmKey: safeText(item.localAlgorithmKey),
